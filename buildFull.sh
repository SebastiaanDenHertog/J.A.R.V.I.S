...
if [ ! -f "CMakeCache.txt" ] || ! grep -q "BUILD_FULL:BOOL=ON" CMakeCache.txt; then
    echo "CMakeCache.txt does not exist or configuration changed. Running CMake..."
    cmake -DTARGET_ARCH=x86_64 -DTARGET_OS=linux -DBUILD_FULL:BOOL=ON -DBUILD_FULL=ON -DDEBUG_MODE=ON -DCMAKE_BUILD_TYPE=Debug -DCMAKE_VERBOSE_MAKEFILE:BOOL=OFF ..
else
