...
<!DOCTYPE HTML><html>
    ...
    <body>
        ...
        <script>
            function updateNetwork(ip, gateway, subnet, hostname){
                var xhr = new XMLHttpRequest();
                xhr.open("GET", "updateNetwork?ip=" + ip + "&gateway=" + gateway + "&subnet=" + subnet + "&hostname=" + hostname, true);
                xhr.send();
            }
...
String WebServer::replacePlaceholder(const String &placeholderName) const
{
    ...
    {
        String network = "";
        network += "<form onsubmit=\"event.preventDefault(); updateNetwork(document.getElementById('ip').value, document.getElementById('gateway').value, document.getElementById('subnet').value, document.getElementById('hostname').value, document.getElementById('ssid').value, document.getElementById('password').value)\">";
        network += "<table>";
    ...
    server.on("/updateNetwork", HTTP_GET, [](AsyncWebServerRequest *request)
              {
                  ...
                  if (request->hasParam("ssid")) {
                      internet.setSSID(request->getParam("ssid")->value());
                  }
                  if (request->hasParam("password")) {
                      internet.setPassword(request->getParam("password")->value());
                  }
