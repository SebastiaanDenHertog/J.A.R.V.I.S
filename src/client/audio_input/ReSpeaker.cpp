...
{
  initBuffer(0x30, 0xf0); sendI2C(databuf,2);//disable global clock
  initBuffer(0x10, 0x48); sendI2C(databuf,2);//disable pll
  initBuffer(0x30, 0xb0); sendI2C(databuf,2);//disable all clocks
  initBuffer(0x21, 0x00); sendI2C(databuf,2);//disable mod clk
  initBuffer(0x22, 0x00); sendI2C(databuf,2);//disable mod rst
}
...
{
  initBuffer(0x33, 0x3f); sendI2C(databuf,2);//sample resolution 16 bit
  initBuffer(0x35, 0x33); sendI2C(databuf,2);//sample resolution 16 bit
  initBuffer(0x60, 0x07); sendI2C(databuf,2);//sample rate 44100 Hz
  initBuffer(0x66, 0x0f); sendI2C(databuf,2);//enable high pass filter
  
  initBuffer(0x14, 0x18); sendI2C(databuf,2);//pll divider config
  initBuffer(0x13, 0x4c); sendI2C(databuf,2);//pll loop divider
  initBuffer(0x12, 0x02); sendI2C(databuf,2);//pll loop divider MSB
  initBuffer(0x11, 0x18); sendI2C(databuf,2);//pll pre divider
  initBuffer(0x18, 0x01); sendI2C(databuf,2);//pll clock lock en
  initBuffer(0x20, 0x89); sendI2C(databuf,2);//pll clock source MCLK

  initBuffer(0x31, 0x03); sendI2C(databuf,2);//bclk div

  initBuffer(0x38, 0x03); sendI2C(databuf,2);//set channels
  initBuffer(0x39, 0x0f); sendI2C(databuf,2);//tx config
  initBuffer(0x3a, 0x00); sendI2C(databuf,2);//tx config
  initBuffer(0x3c, 0x4e); sendI2C(databuf,2);//tx chip map
  initBuffer(0x3d, 0x00); sendI2C(databuf,2);//tx chip map
  initBuffer(0x3e, 0x00); sendI2C(databuf,2);//tx chip map
  initBuffer(0x3f, 0x00); sendI2C(databuf,2);//tx chip map
  
  initBuffer(0x21, 0x93); sendI2C(databuf,2);//mod clk en
  initBuffer(0x22, 0x93); sendI2C(databuf,2);//mod rst ctrl

  initBuffer(0x30, 0xb0); sendI2C(databuf,2);//disable all clocks
  initBuffer(0x30, 0xf0); sendI2C(databuf,2);//enable lrck clock
  initBuffer(0x10, 0x4b); sendI2C(databuf,2);//enable pll
  initBuffer(0x30, 0xf5); sendI2C(databuf,2);//enable global clock
