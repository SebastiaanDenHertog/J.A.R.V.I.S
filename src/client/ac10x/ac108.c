...
static int ac108_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if ((substream->stream == SNDRV_PCM_STREAM_CAPTURE && dai->stream_active[SNDRV_PCM_STREAM_PLAYBACK]) || (substream->stream == SNDRV_PCM_STREAM_PLAYBACK && dai->stream_active[SNDRV_PCM_STREAM_CAPTURE]))
	{
		/* not configure hw_param twice */
		/* return 0; */
	}
	...
	ac108_multi_update_bits(I2S_FMT_CTRL2, 0x07 << SAMPLE_RESOLUTION | 0x07 << SLOT_WIDTH_SEL,
							ac108_samp_res[samp_res].reg_val << SAMPLE_RESOLUTION | ac108_samp_res[samp_res].reg_val << SLOT_WIDTH_SEL, ac10x);
...
static int ac108_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	 ...
	 */
	ac108_multi_update_bits(I2S_FMT_CTRL1, 0x01 << ENCD_SEL | 0x03 << MODE_SEL | 0x01 << TX2_OFFSET | 0x01 << TX1_OFFSET | 0x01 << TX_SLOT_HIZ | 0x01 << TX_STATE,
...
static int ac108_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *i2c_id)
{
	...
	if (of_property_read_u32(np, "tdm-chips-count", &val))
		val = 1;
...
__ret:
	...
	if ((ac10x->codec_cnt != 0 && ac10x->tdm_chips_cnt < 2) || (ac10x->i2c[0] && ac10x->i2c[1] && ac10x->i2c101))
	{
